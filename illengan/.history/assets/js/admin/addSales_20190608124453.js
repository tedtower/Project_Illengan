...
function addAddons(btn) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            <td>
                <input type="number" name="aoQty" id="aoQty" onchange="aoQty setAddOnSubtotal()" value="1" class="form-control form-control-sm" required min="1">
            </td>
            <td>
...
function setSubtotal() {
    $(document).ready(function () {
        ...
        for (var i = 0; i <= elements.length - 1; i++) {
            ...
            olQty = parseInt(document.getElementsClassByName('olQty')[i].value);
            document.getElementsByClassName('subtotal')[i].value = olQty * prPrice;
            var subtotal = parseInt(document.getElementsClassByName('subtotal')[i].value);
